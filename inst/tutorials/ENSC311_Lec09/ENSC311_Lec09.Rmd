...
```{r as_tibble_mtcars, exercise=TRUE}

```

```{r as_tibble_mtcars-solution}
class(mtcars)
library(tibble)
...
```{r as_tibble_mtcars_dplyr, exercise=TRUE}

```

```{r as_tibble_mtcars_dplyr-solution}
mtcars_tib <- dplyr::as_tibble(mtcars)
class(mtcars_tib)
```

```{r as_tibble_mtcars_dplyr-code-check}
grade_code()
```
...
```{r single_column_index, exercise=TRUE}

```

```{r single_column_index-solution}
mtcars[ , "mpg"]
mtcars_tib[ , "mpg"]
```

```{r single_column_index-code-check}
grade_code()
```
...
```{r override_c_and_letters, exercise=TRUE}

```

```{r override_c_and_letters-solution}
c(10,20,30)
letters
```

```{r override_c_and_letters-code-check}
grade_code()
```
...
```{r override_c_and_letters_base-setup}
c <- function(...) "I'm sorry. I won't build that vector. You didn't say please."
letters <- "You're out of luck.  The alphabet is on strike!"
```

```{r override_c_and_letters_base, exercise=TRUE}

```

```{r override_c_and_letters_base-solution}
base::c(10,20,30)
base::letters
```

```{r override_c_and_letters_base-code-check}
grade_code()
```
...
```{r excited_sum, exercise=TRUE}

```

```{r excited_sum-solution}
sum <- function(...) {
...
```{r call_excited_sum, exercise=TRUE}

```

```{r call_excited_sum-solution}
sum(1:100)
base::sum(1:100)
```

```{r call_excited_sum-code-check}
grade_code()
```
...
```{r remove_excited_sum, exercise=TRUE}

```

```{r remove_excited_sum-solution}
rm(sum)
sum(1:100)
```

```{r remove_excited_sum-code-check}
grade_code()
```
...
- You can *unload* a package from the `Packages` tab in the lower right pane or R Studio by simply unchecking the associated box.  If you reload the package *after* unloading it, the package stack *will* be reordered!
...
```{r plot_hp_vs_disp_1, exercise=TRUE}

```

```{r plot_hp_vs_disp_1-solution}
plot(mtcars$disp, mtcars$hp)
```

```{r plot_hp_vs_disp_1-code-check}
grade_code()
```
