...
```{r 01_as_tibble_mtcars, exercise=TRUE}

```

```{r 01_as_tibble_mtcars-solution}
class(mtcars)
library(tibble)
...
```{r 01_as_tibble_mtcars-code-check}
grade_code()
```
...
```{r 02_as_tibble_mtcars_dplyr, exercise=TRUE}

```

```{r 02_as_tibble_mtcars_dplyr-solution}
mtcars_tib <- dplyr::as_tibble(mtcars)
class(mtcars_tib)
```

```{r 02_as_tibble_mtcars_dplyr-code-check}
grade_code()
```
...
```{r 03_single_column_index, exercise=TRUE}

```

```{r 03_single_column_index-solution}
mtcars[ , "mpg"]
mtcars_tib[ , "mpg"]
```

```{r 03_single_column_index-code-check}
grade_code()
```
...
```{r 04_override_c_and_letters, exercise=TRUE}

```

```{r 04_override_c_and_letters-solution}
c(10,20,30)
letters
```

```{r 04_override_c_and_letters-code-check}
grade_code()
```
...
```{r 05_override_c_and_letters_base-setup}
c <- function(...) "I'm sorry. I won't build that vector. You didn't say please."
letters <- "You're out of luck.  The alphabet is on strike!"
```

```{r 05_override_c_and_letters_base, exercise=TRUE}

```

```{r 05_override_c_and_letters_base-solution}
base::c(10,20,30)
base::letters
```

```{r 05_override_c_and_letters_base-code-check}
grade_code()
```
...
```{r 06_excited_sum, exercise=TRUE}

```

```{r 06_excited_sum-solution}
sum <- function(...) {
...
```{r 07_call_excited_sum, exercise=TRUE}

```

```{r 07_call_excited_sum-solution}
sum(1:100)
base::sum(1:100)
```

```{r 07_call_excited_sum-code-check}
grade_code()
```
...
```{r 08_remove_excited_sum, exercise=TRUE}

```

```{r 08_remove_excited_sum-solution}
rm(sum)
sum(1:100)
```

```{r 08_remove_excited_sum-code-check}
grade_code()
```
...
```{r 09_load_dplyr, exercise=TRUE}

```

```{r 09_load_dplyr-solution}
library(dplyr)
```

```{r 09_load_dplyr-code-check}
grade_code()
```
...
```{r 10_plot_hp_vs_disp_1, exercise=TRUE}

```

```{r 10_plot_hp_vs_disp_1-solution}
plot(mtcars$disp, mtcars$hp)
```

```{r 10_plot_hp_vs_disp_1-code-check}
grade_code()
```
