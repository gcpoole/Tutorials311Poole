...
We refer to these relationships as "inheritance."  We say "`numeric` inherits from `atomic`".  By this be mean that `numeric`s have all of the properties of `atomic`s, plus a few more.  More importantly, if you can do a particular operation on an object type, you can do that same operation on any object that inherits from that type.  

For instance, the `+` operator is designed to work on `numeric` vectors.  Therefore, it will work with either `integer`s or `double`s.  Importantly, the `[` operator (for indexing) is designed to work on vectors, so it works on any type of vector.

## The other type of vetor: a `list`.
...
- a vector of `integer`s ranging from 1 to 10 in the second position, and 
...
With only one exception (which we will explore later in the course), R always make copies of `object`s when they are assigned to a name (using `<-`) or placed in a `list`.
...
So we have reached the point where you have all of the knowledge you need to start working with read data sets.  In R, data sets are stored in objects called `data.frame`s.

`data.frame`s contain tabular data.  In other words, `data.frame`s have data arranged in rows and columns.

R has some built-in `data.frame`s.  Consider `mtcars`:
...
What's important to realize, here, is that `data.frame`s are `list`s.  They `list`s with superpowers, but they are `list`s none-the-less.  

In other words, `data.frame`s can do more than a regular `list`.  However, since a `data.frame` is a `list`, everything you just learned about `list`s applies to a `data.frame`.
...
So, if a `data.frame` is a `list` of `vectors` that each represent columns, then what would the names of the `vectors` in the `list` represent? Try it out:
...
Sure! The names in the `list` must be the column names!  And so they are!
...
You can calcuate summary statistic on colums to characterize the data in `mtcars`.
...
## Indexing `data.frame`s

`Data.frame`s are lists.  Therefore, they can be indexed with `[`.  In other words, you can extract a subset of a `data.frame` using the `[` operator.  Because they are lists, they can be indexed with a single vector, just as you have practiced.  Give it a try:
...
- display the three five rows of `three_column_mtcars`
...
However, `data.frame`s have two dimensions, represented by rows and columns.  They can also be indexed in two dimension by putting TWO indexing vectors inside `[ ]` -- the first index subsets the rows and the second subsets the columns.  
...
If you want to include *all* of the columns but still filter the rows, you can leave out the index for the rows, but *be sure to include the comma* to R knows to use two dimensional indexing!  For instance:
...
You can also leave out the row index to include all the rows, although this is the same as indexing with a single vector:
