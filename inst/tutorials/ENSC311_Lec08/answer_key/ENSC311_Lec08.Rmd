...
```{r 01_make_a_list, exercise=tutorialize}
...
```{r 02_display_sum, exercise=tutorialize}
sum
```
...
```{r 03_display_my_list, exercise=tutorialize, exercise.setup=01_make_a_list}
my_list
```
...
```{r 04_shorten_my_list, exercise=tutorialize, exercise.setup=01_make_a_list}
...
```{r 05_length_1_list, exercise=tutorialize, exercise.setup=01_make_a_list}
...
```{r 08_extract_integer_vector, exercise=tutorialize, exercise.setup=01_make_a_list}
...
```{r 09_try_extraction, exercise=tutorialize, exercise.setup=01_make_a_list}
#+ identical(1:10, my_list[2])
identical(1:10, my_list[[2]])
```
...
```{r 10_name_my_list, exercise=tutorialize, exercise.setup=01_make_a_list}
names(my_list) <- c("letters", "integers", "sum_function")
my_list
```
...
```{r 11_name_when_creating_list, exercise=tutorialize}
#+ my_list <- list(letters, 1:10, sum)
...
```{r 12_index_extract_by_name, exercise=tutorialize, exercise.setup=10_name_my_list}
...
```{r 13_dollar_vs_double_square, exercise=tutorialize, exercise.setup=10_name_my_list}
...
```{r 15_str_mtcars, exercise=tutorialize}
str(mtcars)
```
...
```{r 16_head_letters_mtcars, exercise=tutorialize}
head(letters)
head(mtcars)
```
...
```{r 17_head_letters_mtcars_10, exercise=tutorialize}
head(letters, 10)
head(mtcars, 10)
```
...
```{r 18_nrow_ncol, exercise=tutorialize}
nrow(mtcars)
ncol(mtcars)
```
...
```{r 19_df_is_list, exercise=tutorialize}
is.list(mtcars)
```
...
```{r 20_names_of_mtcars, exercise=tutorialize}
names(mtcars)
str(mtcars)
```
...
```{r 21_length_vs_ncol, exercise=tutorialize}
length(mtcars)
ncol(mtcars)
```
...
```{r 22_extract_columns, exercise=tutorialize}
mpg_col <- mtcars$mpg
mpg_col
is.vector(mpg_col)
col_6 <- mtcars[[6]]
...
```{r 23_col_length_vs_nrows, exercise=tutorialize, exercise.setup=22_extract_columns}
length(mpg_col)
length(col_6)
nrow(mtcars)
```
...
```{r 24_mean_disp, exercise=tutorialize}
mean(mtcars$disp)
range(mtcars$mpg)
unique(mtcars$cyl)
```
...
```{r 25_index_mtcars_as_list_1, exercise=tutorialize}
...
```{r 26_index_mtcars_as_list_2, exercise=tutorialize}
...
```{r 27_index_mtcars_as_list_3, exercise=tutorialize}
three_column_mtcars <- 
...
```{r 28_index_mtcars_2D_1, exercise=tutorialize}
...
```{r 29_index_mtcars_2D_2, exercise=tutorialize, exercise.setup=28_index_mtcars_2D_1}
...
```{r 30_index_then_extract, exercise=tutorialize}
