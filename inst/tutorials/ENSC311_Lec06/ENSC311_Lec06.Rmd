...
Please introduce yourself to R by entering your name in the following text box:

```{r 00_users_name}
quiz(
  question_text(
    "Please enter your full name (typically in the US, your first and last name):",
    answer_fn(tutorialize::submitToTA),
    rows = 1,
    correct = "Thanks. Your names has been entered into the grading database so that we know who is completing the tutorial.  This is the only time you will have to enter your name.  The tutorial system will remember you from now on.")
)
```
...
```{r 01_enter_numeric_literal, exercise=TRUE}

```

```{r 01_enter_numeric_literal-solution}
15.23
```

```{r 01_enter_numeric_literal-code-check}
grade_code()
```

If asked what happened, someone might say "I entered a number and, in response, R printed the number."  But that's not all that happened...  

Rather, you entered 4 Arabic numerals (and a decimal point) and R interpreted that string of numerals to create a value in the computer's memory.  Then R reversed the process and converted the value to numerals, and printed the numerals to the screen so you could read them.
...
OK.  I'm kidding about "Roman numeral mode."  There's no such thing.  I just made it look like R was printing in Roman numerals.  But the point is, there could be Roman numeral mode.  In fact, that would be a great practical joke -- make your friend's copy of R print out all values in Roman numerals!  This would be possible by writing a "package" for R.  (Packages install new functionality.  More on packages later in the course...)
...
```{r 02_try_literals, exercise=TRUE}

```

```{r 02_try_literals-solution}
TRUE
"I am learning R!"
"1776-07-04"
```

```{r 02_try_literals-code-check}
grade_code()
```
...
- R gives (or "passes") the interpreted value inside the parentheses to the function.
...
- because we didn't tell R to do anything with the result, R finally converts the result to some sort of printable form (e.g., Arabic numerals) and displays it on the screen.

Now you try:

- In the sandbox below, instruct R to calculate the square root of `360` and press "run code".

```{r 03_sqrt, exercise=TRUE}
...
Every function has a set of (zero or more) parameters.  When we call a function, we must provide one argument to every parameter.
...
```{r 04_display_sqrt, exercise=TRUE}

```

```{r 04_display_sqrt-solution}
sqrt
```

```{r 04_display_sqrt-code-check}
grade_code()
```
...
### Summary: arguments vs. parameters

Here's a little mnemonic to help you keep "arguments" and "parameters" straight: "Arguments are actual; parameters are placeholders."

Let's take stock.

```{r 05_parameter_argument_quiz}
quiz(
  question(
...
```{r 06_params_of_log, exercise=TRUE}

```

```{r 06_params_of_log-solution}
log
```

```{r 06_params_of_log-code-check}
grade_code()
```

Using the function's definition, see if you can answer the following:

```{r 07_log_parameter_quiz}
quiz(
  question(
...
```{r 08_log_function, exercise=TRUE}

```

```{r 08_log_function-solution}
log(100, 10)
```

```{r 08_log_function-code-check}
grade_code()
```
...
```{r, 11_google_exponential_function_quiz}
quiz(
  question(
...
```{r 12_plus_operator, exercise=TRUE}
2+2
```
...
```{r 13_try_operators, exercise=TRUE}

```

```{r 13_try_operators-solution}
6*4
10/2
2^3
```

```{r 13_try_operators-code-check}
grade_code()
```
...
```{r 14_play_with_parentheses, exercise=TRUE}
5 + 4 * 5
```

```{r 14_play_with_parentheses-solution}
(5 + 4) * 5
```

```{r 14_play_with_parentheses-code-check}
grade_code()
```
...
```{r 15_sum_functions, exercise=TRUE}

```

```{r 15_sum_functions-solution}
log(100, 10) + sqrt(64)
```

```{r 15_sum_functions-code-check}
grade_code()
```
...
```{r 16_square_root_squared, exercise=TRUE}

```

```{r 16_square_root_squared-solution}
root <- sqrt(136)
root
root_squared <- root^2
root_squared
...
```{r 16_square_root_squared-code-check}
grade_code()
```
...
but using the name `root` and `squared_root` gave an indication of the value the name were associated with.  When your code gets complex or after you've left it for a while and come back to it, names like `x` and `y` are not helpful for being able to read your code and understand the role of each variable therein.
...
Great job!  You've completed the assignment for Lecture 6!  Your answers have been recorded in a database while you have
been doing the quiz.

If you can't see a table of content to your left, please make the tutorial wider on your screen until you can see the 
table of contents.  Then, take a screen shot or a photo of the completed table of contents and turn in the photo to D2L.
