...
```{r 01_enter_numeric_literal, exercise = tutorialize}
15.23
```
...
```{r 02_try_literals, exercise = tutorialize}
TRUE
"I am learning R!"
"1776-07-04"
```
...
```{r 03_sqrt, exercise = tutorialize}
sqrt(360)
```
...
```{r 04_display_sqrt, exercise = tutorialize}
sqrt
```
...
```{r 06_params_of_log, exercise = tutorialize}
log
```
...
```{r 08_log_function, exercise = tutorialize}
log(100, 10)
```
...
```{r 10_whats_exp1, exercise = tutorialize}
#+ exp(1)
```
...
```{r 12_plus_operator, exercise = tutorialize}
#+ 2+2
```
...
```{r 13_try_operators, exercise = tutorialize}
6*4
10/2
2^3
```
...
```{r 14_play_with_parentheses, exercise = tutorialize}
#+ 5 + 4 * 5
(5 + 4) * 5
```
...
```{r 15_sum_functions, exercise = tutorialize}
log(100, 10) + sqrt(64)
```
...
```{r 16_square_root_squared, exercise = tutorialize}
root <- sqrt(136)
root
root_squared <- root^2
root_squared
