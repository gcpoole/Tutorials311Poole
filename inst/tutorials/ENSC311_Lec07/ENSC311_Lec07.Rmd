...
```{r 01_vector_using_c, exercise=TRUE}

```

```{r 01_vector_using_c-solution}
my_vector <- c(5, 6, 7, 8, 9)
my_vector
```

```{r 01_vector_using_c-code-check}
grade_code()
```
...
```{r 02_vector_using_colon_1, exercise=TRUE}

```

```{r 02_vector_using_colon_1-solution}
-10:10
```

```{r 02_vector_using_colon_1-code-check}
grade_code()
```

What happens when you reverse the numbers?

```{r 03_vector_using_colon_2, exercise=TRUE}

```

```{r 03_vector_using_colon_2-solution}
10:-10
```

```{r 03_vector_using_colon_2-code-check}
grade_code()
```
...
```{r 04_vector_using_seq_1, exercise=TRUE}

```

```{r 04_vector_using_seq_1-solution}
seq(30, 40, 2)
```

```{r 04_vector_using_seq_1-code-check}
grade_code()
```
...
```{r 05_vector_using_seq_2, exercise=TRUE, exercise.reveal_solution=FALSE}
seq(40, 30, 2)
```

```{r 05_vector_using_seq_2-solution}
seq(40, 30, -2)
```

```{r 05_vector_using_seq_2-code-check}
grade_code()
```

What about fractional increments? 

- call `seq()` with `from = 30`, `to = 40`, and `by = 2.2`.  Pay special attention to the final number in the resulting sequence.
...
quiz(
  question("Why is the final value of the last sequence 38.8 instead of 40?",
    ...
    answer("Because 2.2 times 6 is greater than (40 - 30)"),
...
```{r 07_pass_explicit_parameters, exercise=TRUE}
seq(by = ___, to = ___, from = ___)
```

```{r 07_pass_explicit_parameters-solution}
seq(by = 2, to = 10, from = 0)
```

```{r 07_pass_explicit_parameters-code-check}
grade_code()
```
...
```{r 08_pass_mixed_parameters, exercise=TRUE}
seq(by = ___, ___, ___)
```

```{r 08_pass_mixed_parameters-solution}
seq(by = 2, 0, 10)
```

```{r 08_pass_mixed_parameters-code-check}
grade_code()
```
...
```{r 09_fixed_length_seq, exercise=TRUE}

```

```{r 09_fixed_length_seq-solution}
seq(15.23, 30.24, length.out = 10)
```

```{r 09_fixed_length_seq-code-check}
grade_code()
```
...
```{r 10_rep_a_value, exercise=TRUE}

```

```{r 10_rep_a_value-solution}
rep(10, 20)
```

```{r 10_rep_a_value-code-check}
grade_code()
```
...
```{r 11_rep_and_colon, exercise=TRUE}

```

```{r 11_rep_and_colon-solution}
rep(1:5, 3)
```

```{r 11_rep_and_colon-code-check}
grade_code()
```
...
```{r 12_rep_and_c, exercise=TRUE, exercise.reveal_solution=FALSE}

```

```{r 12_rep_and_c-solution}
rep(c(5,9,2), 4)
```

```{r 12_rep_and_c-code-check}
grade_code()
```
...
```{r 13_sample_letters, exercise=TRUE}

```

```{r 13_sample_letters-solution}
some_letters <- sample(letters, 5)
some_letters
```

```{r 13_sample_letters-code-check}
grade_code()
```
...
```{r 14_sample_normal, exercise=TRUE}

```

```{r 14_sample_normal-solution}
random_normal <- rnorm(10000, mean = 3, sd = 6)
hist(random_normal)
```

```{r 14_sample_normal-code-check}
grade_code()
```
...
```{r 15_every_other_letter, exercise=TRUE}

```

```{r 15_every_other_letter-solution}
letters[seq(2,26, 2)]
```

```{r 15_every_other_letter-code-check}
grade_code()
```
...
Try running the following code.  You will get an error.  Does the error make sense?

Now:

- Fix the code so that it returns the 13th, 19th, and 21st letter of the alphabet.

```{r 16_index_MSU, exercise=TRUE}
LETTERS[13, 19, 21]
```

```{r 16_index_MSU-solution}
LETTERS[c(13, 19, 21)]
```

```{r 16_index_MSU-code-check}
grade_code()
```
...
```{r 17_reverse_letters, exercise=TRUE}

```

```{r 17_reverse_letters-solution}
letters[26:1]
```

```{r 17_reverse_letters-code-check}
grade_code()
```
...
```{r 18_be_happy, exercise=TRUE}

```

```{r 18_be_happy-solution}
letters[c(4,15,14,20,23,15,18,18,25,2,5,8,1,16,16,25)]
```

```{r 18_be_happy-code-check}
grade_code()
```

### Those pesky bracketed numbers
...
In the sandbox below, I've provided code that will create a vector of 5 random numbers, assign it to the name `random_values` and then display the values.  Add code to:

- create a logical vector that contains TRUE for values that are less than 0.5 and FALSE for other values. Assign the logical vector to the name `is_less_than_half`
- display the logical vector
- index `random_values` using the logical vector (i.e., enter `random_value[is_less_than_half]`)

```{r 19_is_less_than_half, exercise=TRUE}
random_values <- runif(5)
random_values
```

```{r 19_is_less_than_half-solution}
random_values <- runif(5)
random_values
...
```{r 19_is_less_than_half-code-check}
grade_code()
```
...
In other words, every time you run the code, `random_values` takes on a different set of random numbers.  But regardless of what numbers are contained in `random_vales`, R selects all of the numbers from `random_value` that are less than 0.5!!
...
```{r 20_letter_order, exercise=TRUE}

```

```{r 20_letter_order-solution}
letter_order <- 1:26
letter_order
...
```{r 21_student_grades, exercise=TRUE}
student_grades <- c(89.2, 70.0, 88.2, 99.6, 62.1)
```

```{r 21_student_grades-solution}
student_grades <- c(89.2, 70.0, 88.2, 99.6, 62.1)
...
```{r 21_student_grades-code-check}
grade_code()
```
...
```{r 22_student_grades_index-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r 22_student_grades_index, exercise=TRUE}

```

```{r 22_student_grades_index-solution}
grade_bently <- student_grades["Bently"]
grade_bently
```

```{r 22_student_grades_index-code-check}
grade_code()
```
...
```{r 23_grades_index_lower_case-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r 23_grades_index_lower_case, exercise=TRUE}

```

```{r 23_grades_index_lower_case-solution}
student_grades["bently"]
```

```{r 23_grades_index_lower_case-code-check}
grade_code()
```
...
```{r 25_grades_index_two_names-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r 25_grades_index_two_names, exercise=TRUE}

```

```{r 25_grades_index_two_names-solution}
student_grades[c("Chloe", "Ellen")]
```

```{r 25_grades_index_two_names-code-check}
grade_code()
```
...
```{r 26_more_student_names, exercise=TRUE}

```

```{r 26_more_student_names-solution}
more_student_grades
more_student_names <- names(more_student_grades)
more_student_names
```

```{r 26_more_student_names-code-check}
grade_code()
```

The `class()` function shows us the class of a vector (or other R object). In the following sandbox:

- pass `more_student_grades` to the `class()` function and see what happens.

```{r 27_more_student_grades_class-setup}
dummy <- set.seed(330330)
more_student_grades <- 
  structure(
    runif(50) * 30 + 70,
...
```{r 27_more_student_grades_class, exercise=TRUE}

```

```{r 27_more_student_grades_class-solution}
class(more_student_grades)
```

```{r 27_more_student_grades_class-code-check}
grade_code()
```
...
```{r 28_more_student_names_class-setup}
dummy <- set.seed(330330)
more_student_grades <- 
  structure(
    runif(50) * 30 + 70,
...
```{r 28_more_student_names_class, exercise=TRUE}

```

```{r 28_more_student_names_class-solution}
class(more_student_names)
```

```{r 28_more_student_names_class-code-check}
grade_code()
```
...
```{r 30_names_before_e, exercise=TRUE}

```

```{r 30_names_before_e-solution}
names(more_student_grades) < "E"
```

```{r 30_names_before_e-code-check}
grade_code()
```
...
```{r 31_grades_for_A_to_M, exercise=TRUE}

```

```{r 31_grades_for_A_to_M-solution}
my_student_grades <- more_student_grades[names(more_student_grades) < "N"]
mean(my_student_grades)
```

```{r 31_grades_for_A_to_M-code-check}
grade_code()
```
...
```{r 32_vector_math_1, exercise=TRUE}

```

```{r 32_vector_math_1-solution}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
one_to_ten_a + one_to_ten_b
```

```{r 32_vector_math_1-code-check}
grade_code()
```
...
```{r 33_vector_math_2-setup}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
```

```{r 33_vector_math_2, exercise=TRUE}

```

```{r 33_vector_math_2-solution}
one_to_ten_a * one_to_ten_b
one_to_ten_a / one_to_ten_b
one_to_ten_a - one_to_ten_b
one_to_ten_a^one_to_ten_b
...
```{r 34_recycling_1-setup}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
```

```{r 34_recycling_1, exercise=TRUE}

```

```{r 34_recycling_1-solution}
one_to_ten_a + c(1,2)
```

```{r 34_recycling_1-code-check}
grade_code()
```
...
```{r 35_recycling_2-setup}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
```

```{r 35_recycling_2, exercise=TRUE}

```

```{r 35_recycling_2-solution}
one_to_ten_a + 1:4
```

```{r 35_recycling_2-code-check}
grade_code()
```
...
Yes, `20` creates a vector of length 1:
...
Wow.  That's a lot of learning packed into one tutorial!  Congratulations on finishing!  If you feel like you can't retain it all, don't worry to much.  You'll get to opportunity to revisit and apply these ideas many times before the course is over!  
