...
```{r 01_vector_using_c, exercise = tutorialize}
...
```{r 02_vector_using_colon_1, exercise = tutorialize}
-10:10
```

What happens when you reverse the numbers?

```{r 03_vector_using_colon_2, exercise = tutorialize}
10:-10
```
...
```{r 04_vector_using_seq_1, exercise = tutorialize}
seq(30, 40, 2)
```
...
```{r 05_vector_using_seq_2, exercise = tutorialize, exercise.reveal_solution = FALSE}
#+ seq(40, 30, 2)
seq(40, 30, -2)
```
...
```{r 07_pass_explicit_parameters, exercise = tutorialize}
...
```{r, 08_pass_mixed_parameters, exercise = tutorialize}
#+ seq(by = ___, ___, ___)
seq(by = 2, 0, 10)
```
...
```{r, 09_fixed_length_seq, exercise = tutorialize}
...
```{r, 10_rep_a_value, exercise = tutorialize}
rep(10, 20)
```
...
```{r, 11_rep_and_colon, exercise = tutorialize}
rep(1:5, 3)
```
...
```{r, 12_rep_and_c, exercise = tutorialize, exercise.reveal_solution = FALSE}
rep(c(5,9,2), 4)
```
...
```{r 13_sample_letters, exercise = tutorialize}
...
```{r 14_sample_normal, exercise = tutorialize}
...
```{r 15_every_other_letter, exercise = tutorialize}
letters[seq(2,26, 2)]
```
...
```{r 16_index_MSU, exercise = tutorialize}
#+ LETTERS[13, 19, 21]
LETTERS[c(13, 19, 21)]
```
...
```{r 17_reverse_letters, exercise = tutorialize}
letters[26:1]
```
...
```{r 18_be_happy, exercise = tutorialize}
...
```{r 19_is_less_than_half, exercise = tutorialize}
...
```{r 20_letter_order, exercise = tutorialize}
letter_order <- 1:26
letter_order
...
```{r 21_student_grades, exercise=tutorialize}
...
```{r 22_student_grades_index, exercise=tutorialize, exercise.setup=21_student_grades}
...
```{r 23_grades_index_lower_case, exercise=tutorialize, exercise.setup=21_student_grades}
...
```{r 25_grades_index_two_names, exercise=tutorialize, exercise.setup=21_student_grades}
...
```{r 26_more_student_names, exercise=tutorialize}
more_student_grades
...
```{r 27_more_student_grades_class, exercise=tutorialize, exercise.setup=26_more_student_names}
...
```{r 28_more_student_names_class, exercise=tutorialize, exercise.setup=26_more_student_names}
...
```{r 30_names_before_e, exercise=tutorialize, exercise.setup=26_more_student_names}
...
```{r 31_grades_for_A_to_M, exercise=tutorialize, exercise.setup=26_more_student_names}
...
```{r 32_vector_math_1, exercise=tutorialize}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
...
```{r 33_vector_math_2, exercise=tutorialize, exercise.setup=32_vector_math_1}
one_to_ten_a * one_to_ten_b
one_to_ten_a / one_to_ten_b
one_to_ten_a - one_to_ten_b
one_to_ten_a^one_to_ten_b
...
```{r 34_recycling_1, exercise=tutorialize, exercise.setup=32_vector_math_1}
one_to_ten_a + c(1,2)
```
...
```{r 35_recycling_2, exercise=tutorialize, exercise.setup=32_vector_math_1}
one_to_ten_a + 1:4
```
...
<!-- For now, just relax and turn in your homework using `ENSC311::submit_ENSC311()`.  When you do so, look carefully at the list of options to turn in.  They will be in alphabeticaly not chronilogical order...  -->
